...
type ExtensionIdentifier = any;
type ITextModel = any;
type IMarkerData = any;
type ISingleEditOperation = any;
type IMarkdownString = any;
type EndOfLineSequence = any;
...
export const symbolKindToCssClass = (function() {
	...
	return function toCssClassName(kind: SymbolKind, inline?: boolean): string {
		return `symbol-icon ${inline ? "inline" : "block"} ${_fromMapping[kind] ||
			"property"}`;
	};
})();
...
export function isResourceFileEdit(thing: any): thing is ResourceFileEdit {
	return (
		isObject(thing) &&
		(Boolean((<ResourceFileEdit>thing).newUri) ||
			Boolean((<ResourceFileEdit>thing).oldUri))
	);
}
...
export function isResourceTextEdit(thing: any): thing is ResourceTextEdit {
	return (
		isObject(thing) &&
		(<ResourceTextEdit>thing).resource &&
		Array.isArray((<ResourceTextEdit>thing).edits)
	);
}
...
export interface ITokenizationRegistry {
	...
	registerPromise(
		language: string,
		promise: Thenable<ITokenizationSupport>
	): IDisposable;
