...
export class Emitter<T> {
	...
	get event(): Event<T> {
		if (!this._event) {
			this._event = (listener: (e: T) => any, thisArgs?: any, disposables?: IDisposable[]) => {
				...
				if (this._leakageMon) {
					removeMonitor = this._leakageMon.check(this._listeners.length);
				}
				...
				result = {
					dispose: () => {
						...
						if (!this._disposed) {
							this._listeners.splice(remove - 1, 1);
	...
	fire(event: T): void {
		if (this._listeners) {
			...
			for (let i = 0, e = null; i < this._listeners.length; i++) {
...
export class AsyncEmitter<T extends IWaitUntil> extends Emitter<T> {
	...
	async fireAsync(eventFn: (thenables: Promise<any>[], listener: Function) => T): Promise<void> {
		...
		for (let i = 0, e = null; i < this._listeners.length; i++) {
			...
			this._asyncDeliveryQueue.push([e, eventFn(thenables, typeof e === 'function' ? e : e[0]), thenables]);
		}
